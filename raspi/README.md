...
`start-robot.sh` needs to in the `pigmice-vision` home directory (this is where
robot-vision is as well). To run the program you can type

    $ cp raspi/start-robot.sh ~
However, the code should run automatically (as written in pigmice-vision.service)

All error messages are saved into `robot_vision_output.txt` it's a bit easier
to view and will save them, rather than in the terminal shell. To view these
errors cat the file.
